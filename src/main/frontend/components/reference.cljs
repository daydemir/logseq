...
(rum/defc typed-filter-dialog-inner < rum/reactive
  [filters-atom _close-fn typed-references page-name]
  [:div.filters
...
(defn typed-filter-dialog
  [filters-atom typed-references page-name]
  (fn [close-fn]
    (typed-filter-dialog-inner filters-atom close-fn typed-references page-name)))

(defn- typed-block-with-ref-level
  [block level]
                ...
                (fn [blocks]
                  (map (fn [block]
                         (let [level (inc level)
                           ...
                           (typed-block-with-ref-level block level))) blocks)))
        (assoc :ref/level level))
    (assoc block :ref/level level)))

(defn- typed-blocks-with-ref-level
  [page-blocks]
  (map (fn [[page blocks]]
         [page (map #(typed-block-with-ref-level % 1) blocks)])
    page-blocks))

(rum/defcs typed-references < rum/reactive db-mixins/query
  (rum/local nil ::n-ref)
  {:init (fn [state]
  ...
  (when page-name
    (let [page-name (string/lower-case page-name)
          ...
          *n-ref (::n-ref state)
          ...
          ;; arrow-blocks (model-db/get-matched-blocks "->" 100)
          ;; arrow-block-content (model-db/get-blocks-contents repo arrow-blocks)
          ...
          (when (seq refed-blocks-ids)
            (ui/foldable
             [:div.flex.flex-row.flex-1.justify-between.items-center
              [:h2.font-bold.opacity-50 (str n-ref " Typed Reference"
                                             (when (> n-ref 1) "s") filter-state)]
              [:a.filter.fade-link
               {:title "Filter"
                :on-click (fn []
                            (let [ref-blocks (if block-id
                                  ...
                                  typed-references (db/get-page-linked-refs-refed-pages repo page-name)
                                  typed-references (->> (concat ref-pages typed-references)
                                                  (remove nil?)
                                                  (distinct))]
                              (state/set-modal! (typed-filter-dialog filters-atom  typed-references page-name))))}
